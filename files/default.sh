...
wlan_virtual=1
for ssid2 in ${ssid}
  do sed "s/\[VIRTUALINTERFACE\]/${wlan_ap}_$((wlan_virtual++))/; s/\[SSID\]/${ssid2}/" ${MY_PATH}/hostapd/patatawifi-virtual.conf >> ${MY_PATH}/hostapd/${hostapdconf}
done
...
tmux split-window -h
tmux select-pane -t 0
tmux send-keys "cd ${MY_PATH}/hostapd/; ./hostapd ${hostapdconf}" C-m
